...
type Props = NativeStackScreenProps<RootStackParamList, 'ExploreCareers'>;
...
export default function ExploreCareersScreen({ navigation }: Props) {
  ...
  const loadCareers = async () => {
    try {
      ...
      console.warn('Error loading careers', e);
    } finally {
      setLoading(false);
    }
  };
  ...
  const filtered = useMemo(() => {
    return careersList.filter(career => {
      ...
      const matchLevel = activeLevel === 'all' || career.level.toLowerCase() === activeLevel.toLowerCase();
      const matchSearch =
        !q ||
  ...
  }, [careersList, activeLevel, search]);
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <HeroBanner
          ...
          stats={[
            { value: `${careersList.length || 6}+`, label: 'Career paths' },
        ...
        {filtered.length === 0 && !loading && (
          <View style={styles.empty}>
            ...
            <Text style={styles.emptyText}>No matching careers found.</Text>
          </View>
        )}
